...
extra_tiny_font = pygame.font.SysFont("Arial", 13)
...
class Button:
    ...
    def draw(self, surface):
        ...
        if self.text in ["Магазин", "Включить музыку", "Выключить музыку"]:
...
class Unit(pygame.sprite.Sprite):
    ...
    def update(self, enemies, enemy_base=None):
        ...
        # Если столкновение с врагом — не двигаемся
        elif any(self.rect.colliderect(enemy.rect) for enemy in enemies):
            pass  # Стоим, сражаемся
        # Если у базы врага — атакуем её
            ...
            if self.attack_cooldown <= 0:
                ...
                self.attack_cooldown -= 1
        # Если стоим у базы врага — не двигаемся дальше
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, enemy_type):
        ...
        self.rect = self.image.get_rect(
            center=(SCREEN_WIDTH - 50, SCREEN_HEIGHT // 2 + random.randint(-100, 100)))

        self.target_x = 50

    def update(self, player_base, units=None):
        # Движение к базе игрока
        blocked = False
        if units:
            for unit in units:
                if self.rect.colliderect(unit.rect):
                    ...
                    if self.attack_cooldown <= 0:
                        ...
                        self.attack_cooldown -= 1
        # Если стоим у базы игрока — не двигаемся дальше
...
class Game:
    def __init__(self):
        ...
        # Кнопки
        self.buttons = [
            Button(SCREEN_WIDTH // 2 - 100, 300, 200, 60, "Начать игру"),
            Button(SCREEN_WIDTH // 2 - 100, 400, 200, 60, "Обучение"),
        ...
        self.shop_button = Button(SCREEN_WIDTH - 120, 20, 100, 40, "Магазин")
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            # Обработка игрового процесса
                ...
                # Выбор юнита для покупки
                unit_y = SCREEN_HEIGHT - 60
                for i, unit_type in enumerate(self.unit_types):
                    unit_rect = pygame.Rect(50 + i * 120, unit_y, 100, 40)
                    if unit_rect.collidepoint(mouse_pos) and event.type == MOUSEBUTTONDOWN:
    ...
    def spawn_enemy(self):
        ...
        # Увеличиваем вероятность сильных врагов с каждой волной
    ...
    def draw(self):
        ...
        if self.state == MENU:
            ...
            if self.shield_timer > 0:
                ...
                shield_text = tiny_font.render(f"Щит: {shield_sec} сек", True, BLUE)
                screen.blit(shield_text, (650, 20))

            self.shop_button.draw(screen)

            # Панель выбора юнитов
            pygame.draw.rect(screen, (180, 180, 180), (0, SCREEN_HEIGHT - 80, SCREEN_WIDTH, 80))
            unit_y = SCREEN_HEIGHT - 60
            ...
            for i, unit_type in enumerate(self.unit_types):
                ...
                rect = pygame.Rect(50 + i * 120, unit_y, 100, 40)
                pygame.draw.rect(screen, color, rect, border_radius=5)
                pygame.draw.rect(screen, BLACK, rect, 2, border_radius=5)

                text = tiny_font.render(f"{unit_type} ({unit_cost})", True, BLACK)
            ...
            screen.blit(money_text, (SCREEN_WIDTH // 2 - money_text.get_width() // 2, 50))
